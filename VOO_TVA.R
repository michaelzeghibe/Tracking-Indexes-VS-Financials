...
# Merge (common overlapping period only)
prices <- arkk %>%
...
pairs <- list(
  c("VOO", "ARKK"), c("VOO", "QQQ"), c("VOO", "MAGS"), c("VOO", "VIX"),
  c("ARKK", "QQQ"), c("ARKK", "MAGS"), c("QQQ", "MAGS")
)
...
ggplot(plot_data, aes(x = Date, y = Correlation, color = Label)) +
  ...
  labs(title = "Rolling Correlations with VOO (Monthly)",
